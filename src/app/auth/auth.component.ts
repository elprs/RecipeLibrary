...
export class AuthComponent {
  ...
  onSubmit(form: NgForm) {
    ...
    if (this.isLoginMode) {
      ...
      //subscribe the user
      this.authService.signup(email, password, true).subscribe(
        (resData) => {
          console.log(resData);
          this.isLoading = false;
        },
        (errorMessage) => { // an observable with the error message returning from the authService dince we subscribed
